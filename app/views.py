	...
	if request.method == 'POST':
		if not 'username' in session:
			...
			if session['tipo']!=2:
				...
				cur2.execute(sql)
				dato = cur2.fetchone()
				if dato == None and nombre == "":
					return render_template("noexiste.html")
				elif dato == None and nombre != "":
	...
	if request.method == 'POST':
		if not 'username' in session:
			...
			if session['tipo']!=2:
				...
				sql = """update reservas set jugador1 = NULL, jugador2 = NULL, invitado1 = NULL ,tx1 = NULL , tipo_reserva = NULL, pago = NULL, fecha_reserva = NULL where id = '%s'"""%(idrecibida)
				cur2.execute(sql)
				conn.commit()
...
def forgot():
	if request.method == 'POST':
		...
		if(correo2): #!= None
			...
			conn.commit()
			mensaje = "Para reestablecer su contraseña ingrese al siguiente link: "+sirca_url+"/reset2/" + key
